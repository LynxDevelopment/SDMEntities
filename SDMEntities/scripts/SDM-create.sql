    ...
    create table TB_CA_DOMAINCLUSTERFORMATS (
        ...
        primary key (FKAPPLICATION, FKDOMAIN, FKDOMAINCLUSTER, FKMESSAGEFORMAT)
    );
    ...
    create table TB_DOMAINVALUES (
        ...
        primary key (FKNRMAPPLICATION, FKNRMDOMAIN, FKNORM, FKCLTAPPLICATION, FKCLTDOMAIN, FKCLUSTER, ORIGINALVALUE)
    );
    ...
    create table TB_SDM_ASSETS (
        ...
        SECURITYMESSAGE_ID number(19,0),
        CONCATFIELD varchar2(255 char),
        PERCENTAGE_GROUPING long default 0,
        ISCOMPLETED number(1,0) not null,
        NAME varchar2(64 char),
    ...
    create table TB_SDM_MESSAGEFIELDCONFIGS (
        ...
        FKASSETTYPE varchar2(4 char) not null,
        FKPROVIDER number(19,0) not null,
        primary key (FKASSETTYPE, FKPROVIDER, PATH)
    );
    ...
    create table TB_SDM_STCONFIGMSGFLDS (
        ...
        primary key (ID),
        unique (FKSTFIELDCONFIG, FKPROVIDER, FKASSETTYPE, FKENTERPRISE)
    );
    ...
    alter table HS_SDM_ROWS 
        add constraint FK5D76C8D0FC4CA9B4 
        foreign key (FKSTATEFLOW, FKSTATE) 
        references TB_STATES;

    alter table HS_SDM_ROWS 
        add constraint FK_ROW_HISTORIC_JOB 
        foreign key (FKJOB) 
        references TB_SDM_JOBS;

    alter table HS_SDM_VALUES 
        add constraint FK_VALUE_HISTORIC_JOBFIELD 
        foreign key (FKJOBFIELD) 
        references TB_SDM_FIELDS;

    alter table HS_SDM_VALUES 
        add constraint FKE0F0FE596B2D48AB 
        foreign key (FKROW) 
        references HS_SDM_ROWS;
    ...
    alter table TB_SDM_ASSETS 
        add constraint FK_SECURITYASSET_MARKET 
