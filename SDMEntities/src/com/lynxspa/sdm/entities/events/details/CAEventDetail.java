...
@Entity
@Table(name="TB_CA_EVENTDETAILS") 
@org.hibernate.annotations.Table(appliesTo="TB_CA_EVENTDETAILS",indexes={
	@Index(name="IDX_EVDETAIL_OPERDATE",columnNames="OPERATIONALDATE"),
	@Index(name="IDX_EVDETAIL_REFID",columnNames="REFID"),
	@Index(name="IDX_EVDETAIL_VERSION",columnNames="VERSION"),
	@Index(name="IDX_EVDETAIL_DELETED",columnNames="ISDELETED")
})
public class CAEventDetail implements VersionableAdapter, CAEventDetailAdapter {
	...
	private Map<String,? extends CAEventDetailExtendedAdapter> extensions=new HashMap<String,CAEventDetailExtended>();
	
	
	public CAEventDetail(){
		this(null);
	}
	public CAEventDetail(String _user){
		super();
	...
	@Id
	@SequenceGenerator(name="CAEVENTDETAIL_GEN", sequenceName="CAEVENTDETAIL_GEN", initialValue=1, allocationSize=1)
	@GeneratedValue(strategy=GenerationType.AUTO, generator="CAEVENTDETAIL_GEN")
	...
	@OneToMany(targetEntity = CAEventDetailExtended.class, mappedBy="mainDetail",fetch=FetchType.LAZY)
	...
	public void addExtension(CAEventDetailExtendedAdapter _extension){
		((Map<String,CAEventDetailExtended>)this.extensions).put(_extension.getExtensionId().toString(),(CAEventDetailExtended)_extension);
	}
